...
SwiftyLoadLetter is a Swift utility package for Apple platforms that fills in the gaps left by the standard library and SwiftUI. It provides composable protocols, type-safe extensions on Foundation and SwiftUI types, system-state enums with built-in UI properties, and SwiftUI view helpers — all designed to reduce boilerplate across every Apple platform.
...
SwiftyLoadLetter's protocols are designed to be composed freely. Conform to one or several depending on what your type needs to express.
...
```swift
let first = PersonNameComponent.givenName.value(from: components) // "Ava"
let formatted = PersonNameComponent.string(components, style: .short) // "Ava N."
```
...
A debug-only `print` replacement that prefixes all output with `[🖨️ SwiftyLoadLetter]` for easy filtering in Xcode's console. Compiles to a no-op in release builds.
